...
func (r *repository) PaymentSuccess(ctx context.Context, paymentStatusReq *pb.PaymentStatusRequest) (*empty.Empty, error) {
	...
	_, err = r.pool.Exec(ctx, queryUpdate, pb.PaymentStatus_SUCCESS, paymentStatusReq.ID)
	if err != nil {
		return &empty.Empty{}, errors.New("error fail payment: " + err.Error())
	}
