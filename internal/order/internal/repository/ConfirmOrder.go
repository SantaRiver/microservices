...
func (r *repository) ConfirmOrder(ctx context.Context, orderReq *pb.Order) (*empty.Empty, error) {
	...
	_, err = r.pool.Exec(ctx, queryUpdate, pb.OrderStatus_CONFIRMED, orderReq.ID)
	if err != nil {
		return &empty.Empty{}, errors.New("error cancel order: " + err.Error())
	}
